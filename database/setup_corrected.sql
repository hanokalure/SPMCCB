...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        -- Drop the old constraint if it exists with a different name
        BEGIN
...
-- Create favourites table with proper constraints
...
-- Add constraints to favourites if they don't exist
DO $$ 
BEGIN
...
-- Add constraints to folders if they don't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- Create song_folders junction table with proper constraints
...
-- Add constraints to song_folders if they don't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
