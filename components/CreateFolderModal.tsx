...
const CreateFolderModal: React.FC<CreateFolderModalProps> = ({
  ...
  const { createFolder, loading } = useSupabase();
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      if (result.error) {
        Alert.alert('Error', result.error.message || 'Failed to create folder. Please try again.');
        return;
      }
      ...
      Alert.alert(
        'Success! 🎉',
        `Folder "${trimmedName}" has been created successfully.`,
        [
          {
            text: 'OK',
            onPress: () => {
  ...
  const handleCancel = () => {
    if (isCreating) return; // Don't allow cancel while creating
    onClose();
  };

  const canCreate = folderName.trim().length >= 2 && !isCreating;
          ...
          >
            ...
            <View style={styles.header}>
              ...
              <Text style={[styles.subtitle, { color: colors.textSecondary, fontSize: Math.max(settings?.fontSize || 16, 14) }]}>
                Choose a name to organize your favorite songs
              </Text>
            </View>
            ...
            <View style={styles.inputSection}>
              ...
              <View style={[
                ...
                <TextInput
                  ...
                  placeholder="e.g., Christmas Songs, Praise & Worship"
                  ...
                  autoCorrect={true}
                  returnKeyType="done"
                  onSubmitEditing={canCreate ? handleCreateFolder : undefined}
                  editable={!isCreating}
                />
                {folderName.length > 0 && !isCreating && (
                  <TouchableOpacity
